...
use crate::interpreter::{Store, exec_stmt};
    ...
    Ok(contents) => {
      ...
      match parse_tokens(&exploded_source, &mut tokens) {
        Ok(()) => match parse(&tokens) {
          Ok(ast) => {
            print_syntax_tree(&ast, 2);
            println!("\n--- Running Semantic Analysis ---");
            let mut analyzer = SemanticAnalyzer::new();
            match analyzer.visit_stmt(&ast) {
              Ok(_) => println!("Semantic analysis passed! No uninitialized variables found."),
              Err(e) => {
                handle_error(e);
                return;
              }
